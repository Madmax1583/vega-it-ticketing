...
def render_mom_analytics_page(ticket_df):
    ...
    fig = go.Figure()
    for series, dash, color in [("This Month", "solid", "#3B82F6"), ("Last Month", "dash", "#94A3B8")]:
        part = trend[trend["series"] == series]
        fig.add_trace(go.Scatter(x=part["day_of_month"], y=part["count"], mode="lines+markers", name=series, line=dict(color=color, width=3, dash=dash)))
    fig.update_layout(height=360, margin=dict(l=10, r=10, t=30, b=10), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', xaxis_title='Day of Month', yaxis_title='Ops Count')
