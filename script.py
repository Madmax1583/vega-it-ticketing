...
# Reusable Metric Renderer Function for Subsegments
def render_system_metrics(df_subset):
    if df_subset.empty:
        st.info("No logs present for the current targeted segment layout.")
        return
        
    total_tickets = len(df_subset)
    resolved_tickets = len(df_subset[df_subset['status'].str.lower() == 'resolved'])
    held_tickets = len(df_subset[df_subset['status'].str.lower() == 'on hold - user busy'])
    open_tickets = len(df_subset[df_subset['status'].str.lower().isin(['open', 'in progress'])])
    
    resolved_df = df_subset[(df_subset['status'].str.lower() == 'resolved') & (df_subset['resolution_time'] > 0)]
...
with tab_analysis:
    ...
    if not df_live.empty:
        # Utilize the dynamic metrics engine locally here too
        render_system_metrics(df_live)
...
# -------------------------------------------------------------------------
# TAB 4: ADVANCED REPORTING CENTER (DETAILED EXPORTS WITH KPI BOXES)
# -------------------------------------------------------------------------
with tab_monthly:
    st.write("")
    ...
    if not df_live.empty:
        ...
        # Sub-tab A: MONTHLY TELEMETRY FILTER
        with exp_tab_month:
            ...
            st.markdown("##### 📈 System Metric Indicators")
            render_system_metrics(filtered_m_df)
            st.write("---")
        ...
        # Sub-tab B: WEEKLY TELEMETRY FILTER
        with exp_tab_week:
            ...
            st.markdown("##### 📈 System Metric Indicators")
            render_system_metrics(filtered_w_df)
            st.write("---")
        ...
        # Sub-tab C: TECHNICIAN PERFORMANCE FILTER
        with exp_tab_tech:
            available_techs = sorted(df_export['attended_by'].dropna().unique().tolist())
            selected_tech = st.selectbox("Select Target Engineer Name:", available_techs, key="sel_t")
            ...
            st.markdown("##### 📈 System Metric Indicators")
            render_system_metrics(filtered_t_df)
            st.write("---")
            
            st.write(f"Showing **{len(filtered_t_df)}** records completed or held by `{selected_tech}`:")
            ...
            st.download_button(f"📥 Download Performance Logs for {selected_tech} (.CSV)", csv_t_detailed, f"it_tech_log_{selected_tech}.csv", "text/csv")

        # Sub-tab D: LOCATION PROFILE FILTER
        with exp_tab_loc:
            available_locs = sorted(df_export['location'].dropna().unique().tolist())
            selected_loc = st.selectbox("Select Target Plant / Sector Site:", available_locs, key="sel_l")
            ...
            st.markdown("##### 📈 System Metric Indicators")
            render_system_metrics(filtered_l_df)
            st.write("---")
            
            st.write(f"Showing **{len(filtered_l_df)}** infrastructure logs tracked at `{selected_loc}`:")
            ...
            st.download_button(f"📥 Download Infrastructure Logs for {selected_loc.replace(' ', '_')} (.CSV)", csv_l_detailed, f"it_location_log_{selected_loc.replace(' ', '_')}.csv", "text/csv")
    else:
        st.info("No active production records mapped inside cloud nodes or database is empty.")
