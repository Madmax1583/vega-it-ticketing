...
# Sidebar Navigation & Language Toggle Selector
...
# ===================== LOG NEW TICKET (INTERACTIVE MODE) =====================
if page == "Log New Ticket":
    ...
    if st.session_state.ticket_submitted:
        ...
        # Multi-Column Layout for the Form Layout Strategy
        ...
        with form_col:
            with st.form("ticket_form", clear_on_submit=True):
                ...
                # Form fields requiring instant feedback must run outside, so this functions as structural submission anchor
                st.caption("⚠️ Use the Live Input Box on the right to trigger real-time AI Assistance.")
                ...
                submit_btn = st.form_submit_button("Submit Ticket Blueprint", type="primary")

        with ai_col:
            st.subheader("🤖 Live Copilot Core")
            complaint = st.text_area("Type Live Complaint Description Here *", height=115, placeholder="e.g., Printer is showing offline state or Outlook PST file size full...")
            ...
            if complaint:
                ...
                if cat in AI_SUGGESTIONS:
                    ...
                    # Styled HTML injection container
        ...
        # Handle Submit Processing
        if submit_btn:
            if not user_name or not complaint or not department or not location:
                st.error("❌ Required fields missing! Please ensure both the user profile data and the active description box are filled out.")
            else:
...
# ===================== KEEP ALL OTHER VIEWS EXACTLY THE SAME =====================
    ...
    if not df_live.empty:
        ...
        st.info("No active production records mapped.")
else:
    st.info("Analytics engine sections active in background.")
