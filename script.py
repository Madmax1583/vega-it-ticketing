...
def add_priority_and_sla(df):
    ...
    # Phase 0 SLA targets (approved): Critical 4h, High 8h, Medium 24h, Low 48h
    sla_hours = out["priority"].map({"Critical": 4, "High": 8, "Medium": 24, "Low": 48}).fillna(24)
    out["sla_hours"] = sla_hours
    # Clock starts at start_time when present, else ticket date (avoids midnight false breaches)
    clock_start = response_anchor.combine_first(created_ts)
    # End: close_time for tickets that have it; otherwise now (open / in-progress live view)
    end_ts = close_anchor.fillna(now)
    resolution_clock_hours = ((end_ts - clock_start).dt.total_seconds() / 3600)
    out["sla_clock_start"] = clock_start
    out["sla_clock_hours"] = resolution_clock_hours.round(2)
    out["sla_breach"] = (resolution_clock_hours > out["sla_hours"]) & clock_start.notna()
...
def build_month_over_month_comparison(df):
    ...
    def _metrics(df_):
        ...
        return {
            ...
            'FRT': frt,
            'SLA %': (
                round((1 - df_.loc[resolved_mask, 'sla_breach'].fillna(False).mean()) * 100, 1)
                if int(resolved_mask.sum()) else 0.0
            ),
...
/* High-contrast text — defeat Streamlit theme wash-out on dark bg */
    :root { --muted: #CBD5E1; --text: #F8FAFC; }
    html, body, .stApp, [data-testid="stAppViewContainer"],
    [data-testid="stAppViewContainer"] * {
        font-family: "Inter", "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif !important;
    }
    /* Force every native heading / subheader / markdown title bright */
    h1, h2, h3, h4, h5, h6,
    ...
    div[data-testid="stMarkdownContainer"] > div > h1,
    div[data-testid="stMarkdownContainer"] > div > h2,
    div[data-testid="stMarkdownContainer"] > div > h3 {
    ...
    /* Captions & secondary body copy — readable slate, not washed-out */
...
def render_section_title(title, subtitle=None):
    """Bright section heading that cannot be washed out by Streamlit theme CSS."""
    sub = f'<div class="v-section-sub">{subtitle}</div>' if subtitle else ''
    st.markdown(f'<div class="v-section-title">{title}</div>{sub}', unsafe_allow_html=True)

def render_block_title(title):
    st.markdown(f'<div class="v-block-title">{title}</div>', unsafe_allow_html=True)
...
def render_dashboard(conn):
    ...
    if page == "Home":
        ...
        render_section_title("IT Operations War Room", "A single-screen executive view with KPI ribbon, operational risk, and management insight blocks.")
        task_df = load_tasks_df(conn)
        vendor_df = load_vendor_followups_df(conn)
        ...
        with mid1:
            render_block_title("Ticket Aging")
        ...
        with mid2:
            render_block_title("Department Risk")
        ...
        render_block_title("Management Insights")
        render_insight_cards(insights_df, columns_count=3)
        render_block_title("Month-over-Month")
